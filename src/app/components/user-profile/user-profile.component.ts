...
export class UserProfileComponent implements OnInit {
  ...
  user: IUserVM = {
    ...
    email: "",
    joinDate: new Date(),
    profileImage: localStorage.getItem("profileImage") as string,
  };
    ...
    this.userService.getUserInfo().subscribe(
      (response) => {
        ...
        this.healthInfo = {
          ...
          nutrition: {
            protein: ((this.user.userData?.calories as number) * 0.2).toFixed(
              0
            ),
  ...
  calculateProgress(): number {
    ...
    // If target is higher than starting weight (weight gain goal)
      ...
      return Math.min(
        Math.max(Math.round((gained / totalToGain) * 100), 0),
        100
      );
    }
  }
  ...
  getActivityDescription(): string {
    const descriptions = {
      ...
      medium:
        "Moderate exercise 3-5 days per week. Regular activities include jogging or light sports.",
      high: "Very active lifestyle with intense exercise and physical activity daily.",
    };

    type ActivityLevel = keyof typeof descriptions;
    const activityLevel =
      (this.healthInfo?.activityLevel as ActivityLevel) || "medium";
  ...
  onFileSelected(event: any): void {
    ...
    if (file) {
      localStorage.setItem("profileImage", `/images/${file.name}`);
